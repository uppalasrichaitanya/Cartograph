...
**Blocker status:** Yes — blocks Milestone 4. Can be designed in parallel with the Python
Parser spec (Component 7) — they don't depend on each other, only on Components 1 and 4.
...
future heuristic analyzers (Milestone 9) will populate additional boundary records into the
same structure.
...
**Blocker status:** Yes — blocks Milestone 5.
